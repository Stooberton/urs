...
function URS.Load()
    ...
    if file.Exists("ulx/restrictions.txt", "DATA") then
        URS.restrictions = util.JSONToTable(file.Read("ulx/restrictions.txt", "DATA"))
    end

    if file.Exists("ulx/limits.txt", "DATA") then
        URS.limits = util.JSONToTable(file.Read("ulx/limits.txt", "DATA"))
    end

    if file.Exists("ulx/loadouts.txt", "DATA") then
        URS.loadouts = util.JSONToTable(file.Read("ulx/loadouts.txt", "DATA"))
    end

    -- Initiallize all tables to prevent errors
    for type, types in pairs(URS.types) do
        if not URS[type] then
            URS[type] = {}
        end

        for k, v in pairs(types) do
            if not URS[type][v] then
                URS[type][v] = {}
            end
        end
    end
...
function URS.Save(n)
    if (n == URS_SAVE_ALL or n == URS_SAVE_RESTRICTIONS) and URS.restrictions then
        file.Write("ulx/restrictions.txt", util.TableToJSON(URS.restrictions))
    end

    if (n == URS_SAVE_ALL or n == URS_SAVE_LIMITS) and URS.limits then
        file.Write("ulx/limits.txt", util.TableToJSON(URS.limits))
    end

    if (n == URS_SAVE_ALL or n == URS_SAVE_LOADOUTS) and URS.loadouts then
        file.Write("ulx/loadouts.txt", util.TableToJSON(URS.loadouts))
    end
end
...
function URS.Check(ply, type, what)
    ...
    if restriction then
        if table.HasValue(restriction, "*") then
            if not (table.HasValue(restriction, group) or table.HasValue(restriction, ply:SteamID())) then
    ...
    elseif table.HasValue(URS.types.limits, type) and URS.limits[type] and (URS.limits[type][ply:SteamID()] or URS.limits[type][group]) then
        ...
        if URS.cfg.overwriteSbox:GetBool() then return true end -- Overwrite sbox limit (ours is greater) 
    end
end
...
timer.Simple(0.1, function()
    ...
    if AdvDupe then
        AdvDupe.AdminSettings.AddEntCheckHook("URSDupeCheck", function(ply, Ent, EntTable) return URS.Check(ply, "advdupe", EntTable.Class) end, function(Hook)
    ...
    if AdvDupe2 then
        hook.Add("PlayerSpawnEntity", "URSCheckRestrictedEntity", function(ply, EntTable)
            if URS.Check(ply, "advdupe", EntTable.Class) == false or URS.Check(ply, "advdupe", EntTable.Model) == false then return false end
        end)
    end
end)
...
function URS.CheckRestrictedPickUp(ply, weapon)
    if URS.cfg.weaponPickups:GetInt() == 2 then
        if URS.Check(ply, "pickup", weapon:GetClass(), true) == false then return false end
    elseif URS.cfg.weaponPickups:GetInt() == 1 then
        if URS.Check(ply, "swep", weapon:GetClass()) == false then return false end
    end
end
