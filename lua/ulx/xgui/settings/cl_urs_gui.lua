...
urs.back = xlib.makepanel{
    parent = xgui.null
}

urs.restrictionlist = xlib.makelistview{
    parent = urs.back,
    x = 5,
    y = 5,
    w = 150,
...
urs.typelist = xlib.makelistview{
    parent = urs.back,
    x = 5,
    y = 81,
    w = 150,
...
urs.itemlist = xlib.makelistview{
    parent = urs.back,
    x = 160,
    y = 5,
    w = 425,
...
urs.removebutton = xlib.makebutton{
    parent = urs.back,
    x = 5,
    y = 306,
    w = 150,
...
urs.removebutton.DoClick = function()
    if urs.restrictionlist:GetSelected()[1]:GetValue(1) == "Restrictions" then
        for item, items in pairs(urs.itemlist:GetSelected()) do
            if not urs.removers[items:GetValue(1)] then
                urs.removers[items:GetValue(1)] = {}
            end
            ...
            if selected then
                if urs.typelist["Lines"] then
                    for line, lines in pairs(urs.typelist["Lines"]) do
                        if lines:GetValue(1) == selected then
